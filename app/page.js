...
// ─── Super admin email — only this account sees the old admin panel ───────────
...
export default function App() {
  ...
  const [landlordSession,   setLandlordSession]  = useState(null);
  const [tenantSession,     setTenantSession]    = useState(null);
  const [properties,        setProperties]       = useState([]);
  const [loading,           setLoading]          = useState(true);
  const [showLogin,         setShowLogin]        = useState(false);
  const [showLandlordLogin, setShowLandlordLogin] = useState(false);
  ...
  // Theme
  useEffect(() => {
    try { const s = localStorage.getItem('lm_dark'); setDarkMode(s !== null ? JSON.parse(s) : false); } catch {}
  }, []);
  useEffect(() => {
  ...
  // Firebase auth — ONLY for super admin
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, user => {
      if (user && SUPER_ADMIN_EMAIL && user.email === SUPER_ADMIN_EMAIL) {
        ...
        if (user) signOut(auth); // sign out any non-admin Firebase users
      }
    });
    return () => unsub();
  }, []);

  // Restore landlord session from sessionStorage
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem('lm_landlord');
      if (stored) {
        const session = JSON.parse(stored);
        if (session?.code && session?.landlordId) {
  ...
  // Restore tenant session from sessionStorage
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem('lm_tenant');
      if (stored) {
        const session = JSON.parse(stored);
        if (isOTPValid(session.expiresAt)) {
  ...
  const toggleAvail  = useCallback(async prop => { await update(ref(db, `properties/${prop.id}`), { available: prop.available === false ? true : false }); }, []);
  const handleLogin  = useCallback(() => { setIsAdmin(true); setIsSuperAdmin(true); setShowLogin(false); setPage('admin'); }, []);
  const handleLogout = useCallback(() => { signOut(auth); setIsAdmin(false); setIsSuperAdmin(false); setPage('home'); }, []);

  // Landlord code-based auth
  const handleLandlordLogin = useCallback((session) => {
    setLandlordSession(session);
    setShowLandlordLogin(false);
    setPage('landlord');
  }, []);
  const handleLandlordLogout = useCallback(() => {
    sessionStorage.removeItem('lm_landlord');
    setLandlordSession(null);
    setPage('home');
  }, []);

  // Tenant auth
  const handleTenantLogin = useCallback((session) => {
    setTenantSession(session);
    setShowTenantLogin(false);
    setPage('tenant');
  }, []);
  const handleTenantLogout = useCallback(() => {
    sessionStorage.removeItem('lm_tenant');
    setTenantSession(null);
    setPage('home');
  }, []);

  const sharedHeaderProps = { darkMode, toggleDarkMode: toggleDark, onOpenAdminLogin: () => setShowLogin(true) };

  return (
    <>
      {/* ── Pages ── */}
      {page==='home'     && <HomePage properties={properties} loading={loading} darkMode={darkMode} toggleDarkMode={toggleDark} onView={viewDetails} onWhatsApp={whatsApp} onOpenLightbox={openLb} onOpenAdminLogin={()=>setShowLogin(true)} onOpenLandlordLogin={()=>setShowLandlordLogin(true)} onOpenTenantLogin={()=>setShowTenantLogin(true)} />}
      ...
      {page==='admin'    && isSuperAdmin && <AdminPage properties={properties} darkMode={darkMode} toggleDarkMode={toggleDark} onLogout={handleLogout} onSave={saveProp} onDelete={delProp} onEdit={p=>setEditProp(p)} onToggleFeatured={toggleFeat} onToggleAvailable={toggleAvail} onOpenLightbox={openLb} onOpenAdminLogin={()=>setShowLogin(true)} />}
      {page==='landlord' && landlordSession && <LandlordDashboard landlordSession={landlordSession} darkMode={darkMode} toggleDarkMode={toggleDark} onLogout={handleLandlordLogout} onOpenAdminLogin={()=>setShowLogin(true)} />}
      {page==='tenant'   && tenantSession && <TenantDashboard tenantSession={tenantSession} darkMode={darkMode} toggleDarkMode={toggleDark} onLogout={handleTenantLogout} onOpenAdminLogin={()=>setShowLogin(true)} />}
      ...
      {showLandlordLogin  && <LandlordLoginModal onLogin={handleLandlordLogin} onClose={()=>setShowLandlordLogin(false)} />}
      {showTenantLogin    && <TenantLoginModal onLogin={handleTenantLogin} onClose={()=>setShowTenantLogin(false)} />}
...
function HomePage({ properties, loading, darkMode, toggleDarkMode, onView, onWhatsApp, onOpenLightbox, onOpenAdminLogin, onOpenLandlordLogin, onOpenTenantLogin }) {
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      <Header currentPage="home" darkMode={darkMode} toggleDarkMode={toggleDarkMode} onNavigateHome={()=>{}} onOpenAdminLogin={onOpenAdminLogin} onOpenLandlordLogin={onOpenLandlordLogin} onOpenTenantLogin={onOpenTenantLogin} />
      ...
      <footer style={{ borderTop:'1px solid var(--border)', padding:'1.25rem 1.5rem' }}>
        <div style={{ maxWidth:'88rem', margin:'0 auto', display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:'0.75rem' }}>
          <p style={{ fontSize:'0.8125rem', color:'var(--text-3)', margin:0 }}>&copy; {new Date().getFullYear()} LodgeMate · Minna, Niger State</p>
          <p style={{ fontSize:'0.8125rem', color:'var(--text-3)', margin:0 }}>All rights reserved</p>
        </div>
      </footer>
    </div>
  );
      ...
      <div style={{ maxWidth:'90rem', margin:'0 auto' }}>
        <div className="anim-up" style={{ marginBottom:'2rem' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'0.875rem', flexWrap:'wrap', marginBottom:'0.5rem' }}>
            <h2 style={{ fontSize:'clamp(1.75rem,3.5vw,2.5rem)', fontWeight:900, letterSpacing:'-0.04em', color:'var(--text)', margin:0, lineHeight:1.1 }}>Listings</h2>
            {!loading && properties.length > 0 && (
              <div style={{ display:'flex', alignItems:'center', gap:'0.5rem', background:'rgba(21,128,61,0.08)', border:'1px solid rgba(21,128,61,0.2)', borderRadius:'9999px', padding:'0.25rem 0.75rem' }}>
                <div className="live-dot" />
                <span style={{ fontSize:'0.8125rem', fontWeight:700, color:'var(--green)' }}>{properties.length} Available</span>
              </div>
            )}
          </div>
          <p style={{ color:'var(--text-2)', fontSize:'0.9375rem', margin:0 }}>Handpicked across Gidan-Kwano, Gidan-Mango, Kpakungun &amp; Gurara</p>
        </div>
